...
public:
	...
	void showEngMeaning() {
		std::cout << "English meaning: " << engMeaning << "\n";
	}
	void showVieMeaning() {
		std::cout << "Vietnamese meaning: " << vieMeaning << "\n";
	}
	...
	Vocab() {
		eng = "This English Word is None";
		engMeaning = "Node English Meaning";
		vieMeaning = "Node Vietnamese Meaning";
	}
...
public:
	...
	// change all capital letters to lower letters
	...
	// 1: complete
	// 2: the word already exists in the list
	...
	// 2: the word already exists in the list
	int addWord() {
		std::cout << "The infomation of the word you want to add:\n";
		std::string engWord, engMeaning, vieMeaning;
		...
		std::cout << "English meaning (be careful, it can be anything): ";
		std::getline(std::cin, engMeaning);
		if (engMeaning.size() == 0) engMeaning = "Node English Meaning";


		std::cout << "Vietnamese meaning (be careful, it can be anything): ";
		std::getline(std::cin, vieMeaning);
		if (vieMeaning.size() == 0) vieMeaning = "Node Vietnamese Meaning";
	...
	// 2: don't have the wanted-word in the list
	int removeWord() {
		std::string engNeedToDelWord = this->suggestWord();
		if (!haveWord(engNeedToDelWord)) return 2;
