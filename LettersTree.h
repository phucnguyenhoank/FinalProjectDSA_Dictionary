...
public:
	...
	Node* addLetter(Node* r, char singleLetter) {
		// don't add a new node at most of the time
		...
		Node* nextNode = r->getLetterAt(singleLetter - 'a');
		...
		if (nextNode == nullptr) {
			...
			r->setLetterAt(singleLetter - 'a', newNode);
			return newNode;
		}
	...
	void pressWord(std::string word) {
		...
		Node* nextNode = root->getLetterAt(fir - 'a');
		// if it hasn't been placed yet
		if (nextNode == nullptr) {
			Node* newNode = new Node();
			root->setLetterAt(fir - 'a', newNode);
			newNode->setKey(fir);
		}

		// continue add next letters of the words
		Node* tempRoot = root->getLetterAt(fir - 'a');
		while (!word.empty()) {
			fir = word[0];
			word.erase(0, 1);
	...
	void remove(std::string word) {
		...
		while (word.size()) {
			...
			nextNode = nextNode->getLetterAt(fir - 'a');
		}
	...
	Node* track(Node* r, std::string mean) {
		...
		Node* nextNode = r->getLetterAt(fir - 'a');
